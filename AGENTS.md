...
Global hotkeys: `Ctrl+Alt+L` toggles the overlay and `Ctrl+Alt+P` pauses or resumes tracking (written `⌃⌥L` / `⌃⌥P` on macOS — `hotkeyLabel` in `shared/.../platform`, and the tray shows them as native accelerators via `KeyShortcut`), both through `nucleus.global-hotkey` and the shared `HotKeyEffect` in `desktopApp/src/main/kotlin/main.kt`, each gated by its own preference (`globalHotkey`, `pauseHotkey`; Settings row + tray checkbox), off by default like autostart. The tray's first item is the same play/pause toggle, its icon following `session.isPaused`. The combo is fixed; the native side maps the AWT key code and the portable modifier flags to Carbon (`ALT` → optionKey, `CONTROL` → controlKey).
